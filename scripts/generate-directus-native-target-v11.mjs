...
const outPath = path.join(root, 'directus/target-schema-v11.json');
...
const fieldSpecs = {
  ...
  project_details: [
    ['id','uuid',{primary:true}], ['content_id','uuid',{required:true,unique:true,relation:'content'}], ['object_type','string',{}], ['location','string',{}], ['client_display_name','string',{}], ['scope','text',{}],
  ...
  service_details: [
    ['id','uuid',{primary:true}], ['content_id','uuid',{required:true,unique:true,relation:'content'}], ['service_family','string',{required:true}], ['service_area','string',{}], ['target_customer','text',{}],
...
const systemFields = [{"collection":"directus_activity","field":"timestamp","schema":{"is_indexed":true}},{"collection":"directus_oauth_clients","field":"date_created","schema":{"is_indexed":true}},{"collection":"directus_oauth_codes","field":"expires_at","schema":{"is_indexed":true}},{"collection":"directus_oauth_codes","field":"used_at","schema":{"is_indexed":true}},{"collection":"directus_oauth_consents","field":"client","schema":{"is_indexed":true}},{"collection":"directus_oauth_tokens","field":"code_hash","schema":{"is_indexed":true}},{"collection":"directus_oauth_tokens","field":"expires_at","schema":{"is_indexed":true}},{"collection":"directus_oauth_tokens","field":"previous_session","schema":{"is_indexed":true}},{"collection":"directus_oauth_tokens","field":"session","schema":{"is_indexed":true}},{"collection":"directus_revisions","field":"activity","schema":{"is_indexed":true}},{"collection":"directus_revisions","field":"parent","schema":{"is_indexed":true}},{"collection":"directus_sessions","field":"oauth_client","schema":{"is_indexed":true}}];

const target = { version:VERSION, directus:DIRECTUS, vendor:VENDOR, collections, fields, systemFields, relations };
await fs.writeFile(outPath, `${JSON.stringify(target,null,2)}\n`);
console.log(`Generated V11 Directus native target: ${collections.length} collections, ${fields.length} fields, ${relations.length} relations.`);
