...
const target = JSON.parse(await fs.readFile(path.join(root,'directus/target-schema-v10.json'),'utf8'));
...
const report={format:'aeroventa-directus-native-target-v10-validation',verdict:failures.length?'FAIL':'PASS',counts:{collections:target.collections?.length??0,fields:target.fields?.length??0,systemFields:target.systemFields?.length??0,relations:target.relations?.length??0},system_fields_policy:'PRESERVE_OBSERVED_PRODUCTION_INDEXES_EXACTLY',failures};
